...
- BallonsTranslator registers `opencv-tela`, but this class currently calls `cv2.inpaint(..., cv2.INPAINT_NS)`. The experiment keeps that registered name and records the routed method as `bt_opencv-tela_actual_cv2_INPAINT_NS`.
...
python experiments/phase6_bt_method_grid.py `
  ...
  --run-id phase6-bt-method-grid-gbc06-18-v2-near-square `
  --env-file .env
```
...
```text
outputs/runs/phase6-bt-method-grid-gbc06-18-v2-near-square/visuals/detector-grid.png
outputs/runs/phase6-bt-method-grid-gbc06-18-v2-near-square/visuals/inpaint-grid.png
```
...
```text
outputs/runs/phase6-bt-method-grid-gbc06-18-v2-near-square/reports/mimo-detector-grid.json
outputs/runs/phase6-bt-method-grid-gbc06-18-v2-near-square/reports/mimo-inpaint-grid.json
outputs/runs/phase6-bt-method-grid-gbc06-18-v2-near-square/reports/bt-method-grid-summary.json
```
...
| --- | --- | --- | --- | --- |
| `ctd` | ok | Found a target block close to the tight cleanup mask and separately found the neighbor bubble. | 9 | Best detector for this case. Good candidate for Phase 2 fallback/refinement. |
| `ysgyolo` | ok | Detected many fragmented/overlapping boxes and merged unrelated vertical text outside the target bubble. | 2 | Not acceptable as direct cleanup mask source without additional filtering. |
...
| --- | --- | ---: | --- |
| `lama_large_512px` | ok | 100 | Best final result in the v2-near-square MIMO run. Cleanest target removal with neighbor text and bottom line art preserved. |
| `patchmatch` | ok | 95 | Strong lightweight fallback. Very clean, slightly behind LaMa large on paper texture integration. |
| `lama_mpe` | ok | 90 | Strong result, slightly behind LaMa large and PatchMatch in edge integration. |
| `aot` | ok | 85 | Usable but leaves faint gray dirt/residue. |
| `opencv-tela` | ok | 10 | Not acceptable here; leaves obvious ghost text. |
...
```json
{
  ...
  "unacceptable_methods": ["opencv-tela"]
}
```

Manual reading of the grid mostly matches the ranking. `opencv-tela` is clearly too weak for this bubble. `patchmatch`, `aot`, `lama_mpe`, and `lama_large_512px` all preserve the left neighbor text and bottom art; `lama_large_512px` is the best candidate for higher-quality output, while `patchmatch` is much faster and good enough as a local fallback.
...
| `opencv-tela` | `bt_opencv-tela_actual_cv2_INPAINT_NS` | `ballontranslator/modules/inpaint/inpaint_default.py` | behavior matched directly | BT registers `opencv-tela`, but the implementation calls `cv2.INPAINT_NS`, not `cv2.INPAINT_TELEA`. |
...
phase6-bt-method-grid-gbc06-18-v2-near-square  current near-square detector + inpaint + MIMO run
```
