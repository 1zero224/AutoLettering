...
def test_run_cta_first_cleanup_pipeline_routes_matched_and_fallback_records(tmp_path: Path, monkeypatch):
    ...
    assert "quality-accepted gpt-image-2" in manifest["photoshop_export_contract"]["repaired_image_source"]
...
def test_run_cta_first_cleanup_pipeline_gates_successful_gpt_replacements_by_quality(tmp_path: Path, monkeypatch):
    def fake_phase2(**kwargs):
        run_dir = Path(kwargs["output_root"]) / kwargs["run_id"]
        run_dir.mkdir(parents=True)
        _write_jsonl(
    ...
    def fake_phase6(**kwargs):
        run_dir = Path(kwargs["output_root"]) / kwargs["run_id"]
        run_dir.mkdir(parents=True)
        replacement = run_dir / "fallback_replacement_crop" / "page-png-2.png"
        replacement.parent.mkdir(parents=True)
        replacement.write_bytes(b"fake png bytes")
        _write_jsonl(
            run_dir / "cleanup-results.jsonl",
            [
                {
    ...
    manifest = json.loads((run_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["phase6_gpt_quality_run_dir"] == str(quality_run)
    assert manifest["routes"][1]["completion_condition"] == (
    ...
    assert manifest["summary"]["gpt_image2_replacement_records"] == 0
    assert manifest["summary"]["gpt_image2_pending_or_failed_records"] == 1
    assert manifest["summary"]["gpt_image2_quality_rejected_records"] == 1
    assert manifest["summary"]["gpt_image2_quality_missing_records"] == 0
